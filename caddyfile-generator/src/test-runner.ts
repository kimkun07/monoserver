...
async function compareOutputs(caseName: string): Promise<{ passed: boolean; message: string; details?: string }> {
  const caseDir = join(process.cwd(), 'test', caseName);
  const expectedFile = join(caseDir, 'expected', 'Caddyfile');
  const actualFile = join(caseDir, 'Caddyfile');
  ...
  try {
    ...
    if (expectedContent !== actualContent) {
      return {
        passed: false,
        message: `Content mismatch in Caddyfile`,
        details: `Expected:\n${'─'.repeat(50)}\n${expectedContent}\n${'─'.repeat(50)}\n\nActual:\n${'─'.repeat(50)}\n${actualContent}\n${'─'.repeat(50)}`,
      };
    }
    ...
    return {
      passed: true,
      message: `Caddyfile matches expected output`,
    };
  } catch (error: any) {
    return {
      passed: false,
...
async function cleanupOutputDir(caseName: string): Promise<void> {
  const caseDir = join(process.cwd(), 'test', caseName);
  const outputFile = join(caseDir, 'Caddyfile');
...
async function main(): Promise<void> {
  console.log('\n' + '═'.repeat(60));
  console.log('caddyfile-generator Test Suite');
  console.log('═'.repeat(60));
