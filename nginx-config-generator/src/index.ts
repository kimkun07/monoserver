...
async function generateNginxConfigs(): Promise<void> {
  try {
    ...
    for (const [serviceName, serviceConfig] of services) {
      ...
      // Skip services without x-monoserver-port (not exposed via nginx)
      if (!serviceConfig['x-monoserver-port']) {
        console.log(`Skipping ${serviceName} (no x-monoserver-port specified)`);
        continue;
      }
      ...
      console.log(`Adding /${serviceName}/ route (port: ${serviceConfig['x-monoserver-port']})`);
      serviceCount++;
    }

    // Combine all location blocks into routes config
    const routesConfig = generateRoutesConfig(locationBlocks);

    console.log(`\nWriting routes.conf...`);
    await writeFile(confFilePath, routesConfig, 'utf-8');
...
function generateLocationBlock(serviceName: string, serviceConfig: ComposeService): string {
  ...
  // x-monoserver-port is required - this function should only be called for services with it
  if (!port) {
    throw new Error(`Service ${serviceName} missing required x-monoserver-port`);
  }
...
function generateRoutesConfig(locationBlocks: string[]): string {
  const header = `# This file is auto-generated by nginx-config-generator
# DO NOT EDIT MANUALLY - changes will be overwritten
# Service-specific location blocks for path-based routing`;
